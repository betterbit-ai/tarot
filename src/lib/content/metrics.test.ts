...
describe("Threads metrics sync", () => {
  it("does not call the API in dry run", async () => {
    ...
    expect(result).toEqual({ mode: "dry-run", updated: 0, failed: [], quarantined: [] });
  ...
  it("stores only metrics returned by Threads", async () => {
    ...
    expect(result).toEqual({ mode: "synced", updated: 1, failed: [], quarantined: [] });
    expect(state.items.one?.metrics).toMatchObject({ views: 12, likes: 3 });
    vi.unstubAllGlobals();
  });

  it("reports retryable provider failures instead of silently skipping them", async () => {
    ...
    await expect(syncThreadsMetrics({ read: async () => state, write: async (next) => { state = next; } }, { apiBaseUrl: "https://graph.threads.net/v1.0", accessToken: "token", metrics: ["views"], dryRun: false })).resolves.toEqual({ mode: "partial", updated: 1, failed: [{ contentId: "two", status: 429 }], quarantined: [] });
    vi.unstubAllGlobals();
  });

  it("quarantines a permanently malformed historical post while syncing the rest", async () => {
    let state: ContentRuntimeQueue = { version: 1, items: {
      one: { status: "PUBLISHED", updatedAt: "2026-09-09T00:00:00.000Z", attemptCount: 1, mainPostId: "post-one" },
      old: { status: "PUBLISHED", updatedAt: "2026-09-09T00:00:00.000Z", attemptCount: 1, mainPostId: "old-post" },
    } };
    vi.stubGlobal("fetch", vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ name: "views", values: [{ value: 12 }] }] }), { status: 200 }))
      .mockResolvedValueOnce(new Response("unsupported historical media", { status: 400 })));

    await expect(syncThreadsMetrics({ read: async () => state, write: async (next) => { state = next; } }, { apiBaseUrl: "https://graph.threads.net/v1.0", accessToken: "token", metrics: ["views"], dryRun: false })).resolves.toEqual({ mode: "synced", updated: 1, failed: [], quarantined: [{ contentId: "old", status: 400 }] });
    expect(state.items.old).toMatchObject({ metricsUnavailableStatus: 400 });
    vi.unstubAllGlobals();
  });
});
