...
function reasonForCategory(category: AffiliateProductCategory, cards: readonly TarotCard[]): string {
  ...
  switch (category) {
    case "love":
      return `${lastCard}까지 보고 나면 말보다 분위기를 바꾸는 쪽이 먼저예요. 관계를 조금 다르게 느껴보고 싶은 날에 어울리는 향을 골랐어요.`;
    case "rest":
      return `${lastCard}가 남긴 여운처럼, 생각을 오래 끌기보다 한 번 끊어 주는 게 필요해 보여요. 익숙한 공간의 공기를 바꿔 줄 짙은 우디 향이에요.`;
    case "career":
      return `${lastCard}가 마지막에 놓였네요. 새 환경에 들어가는 날, 차분하게 준비하고 싶다면 이런 향도 괜찮겠어요.`;
    case "money":
      return "소비를 늘리라는 뜻은 아니에요. 원래 향수를 바꾸려던 참이라면 충동구매 대신 오래 쓸 한 가지를 비교해 보라는 제안에 가까워요.";
    case "energy":
      return `${lastCard}처럼 먼저 움직여야 하는 날에는 분위기를 금세 바꿔 주는 물건이 잘 맞아요. 선명하지만 무겁지 않은 우디 향이에요.`;
    case "general":
    default:
      return "카드를 덮고 일상으로 돌아갈 때, 분위기를 한 번 바꿔 줄 물건을 골랐어요. 짙고 차분한 우디 향수예요.";
  }
}
