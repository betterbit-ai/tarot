...
- Repair workflow run `34239748737` returned 404 before the Vercel deployment propagated and created no comments. Retry `34239915987` succeeded with `mode: repaired`, appending the three prepared readings (펜타클 에이스, 여황제, 심판) under the confirmed `mr-tarot-0008` parent post.
...
- Coupang daily Slack report implementation is staged: it uses only the official aggregate `/reports/commission` for the previous KST day, formats click/order/cancel/GMV/commission plus separately labeled funnel counts, and sends once through a Vercel-only Slack webhook. The GitHub trigger is 15:10 KST. It still requires `COUPANG_PARTNERS_REPORT_ENABLED=true` and `SLACK_WEBHOOK_URL` in Vercel before a live report can be sent.
- Slack/Coupang report was verified live after environment setup: GitHub run `34361447286` returned `mode: sent` for report date `20260908` with click 0, order 0, cancel 0, GMV 0 and commission 0. The scheduled 15:10 KST run can now send the same aggregate format daily without exposing credentials.
...
- The refreshed pool is live behind `/api/affiliate/pool`; the ritual still falls back to the local verified product if Upstash is unavailable.
...
Latest missing-reply repair checkpoint: `eefb5de` — a protected manual workflow can append only the three prepared readings to an already-published post; successful live repair is run `34239915987`.
