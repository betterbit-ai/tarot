...
- `COUPANG_PARTNERS_REPORT_ENABLED=true`: enables the daily aggregate report route
- `SLACK_WEBHOOK_URL`: Slack incoming-webhook URL for the private operations channel
...
`Send Coupang daily report to Slack` runs at 15:10 KST, after the Partners API's stated 15:00 daily update. It requests only the previous KST day's `/reports/commission` aggregate and sends click, order, cancel, GMV, commission, and separately labeled Mr. Tarot funnel counts. The route uses a daily Upstash marker so repeated manual/scheduled calls do not duplicate the Slack message. A failed Slack/API call leaves no sent marker and can be retried. Never place the Slack webhook in GitHub Actions; it belongs only in Vercel Production and Preview secrets.
